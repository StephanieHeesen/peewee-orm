...
def vegetarian_dishes() -> List[models.Dish]:
    ...
    """
    # models.db.create_tables([models.Dish, models.Ingredient, models.DishIngredient])
    listdish = []
    ingrlist = []
    dishes = models.Dish.select()
    for dish in models.Dish.select():
        for ingredient in models.DishIngredient:
            ingrlist.append(models.Ingredient.is_vegetarian.select(ingredient))
        if all(ingrlist):
            listdish.append(dish) 
        return listdish
        
    # query = (models.Dish
    #             .select(models.Dish, models.Ingredient, models.DishIngredient)
    #             .join(models.DishIngredient)
    #             .where(all(ingrlist) == True))
    # return listdish.append(query)
